...
class TestBasicSetup(unittest.TestCase):
    ...
    def test_silver_learning_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/silver/silver_moodle_learning_events.sql").exists())
        self.assertNotIn("ensure_moodle_silver_learning_events_view", database_py)
        self.assertNotIn("ensure_moodle_silver_learning_events_view()", main_py)
    ...
    def test_activity_dimension_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")
        api_py = Path("src/app/api/moodle_logs.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/dim_moodle_course_activities.sql").exists())
        self.assertNotIn("ensure_moodle_course_activities_view", database_py)
        self.assertNotIn("ensure_moodle_course_activities_view()", main_py)
        self.assertIn('@router.get("/activity-dim-summary")', api_py)

    def test_gold_user_summary_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")
        api_py = Path("src/app/api/moodle_logs.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_user_learning_summary.sql").exists())
        self.assertNotIn("ensure_gold_user_learning_summary_view", database_py)
        self.assertNotIn("ensure_gold_user_learning_summary_view()", main_py)
        self.assertIn('@router.get("/gold-user-summary")', api_py)

    def test_gold_registered_user_summary_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")
        api_py = Path("src/app/api/moodle_logs.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_registered_user_learning_summary.sql").exists())
        self.assertNotIn("ensure_gold_registered_user_learning_summary_view", database_py)
        self.assertNotIn("ensure_gold_registered_user_learning_summary_view()", main_py)
        self.assertIn('@router.get("/gold-registered-user-summary")', api_py)

    def test_gold_team_summary_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")
        api_py = Path("src/app/api/moodle_logs.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_team_learning_summary.sql").exists())
        self.assertNotIn("ensure_gold_team_learning_summary_view", database_py)
        self.assertNotIn("ensure_gold_team_learning_summary_view()", main_py)
        self.assertIn('@router.get("/gold-team-summary")', api_py)

    def test_gold_individual_summary_model_is_managed_by_dbt(self):
        database_py = Path("src/app/core/database.py").read_text(encoding="utf-8")
        main_py = Path("src/app/main.py").read_text(encoding="utf-8")
        api_py = Path("src/app/api/moodle_logs.py").read_text(encoding="utf-8")

        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_individual_learning_summary.sql").exists())
        self.assertNotIn("ensure_gold_individual_learning_summary_view", database_py)
        self.assertNotIn("ensure_gold_individual_learning_summary_view()", main_py)
    ...
    def test_learning_dashboard_overview_endpoint_is_available(self):
        ...
        self.assertIn("daily_interactions", api_py)
        ...
        self.assertIn("use_analytics_schema_if_available", api_py)
        self.assertIn("gold_daily_learning_interactions", api_py)
        self.assertIn("gold_key_activity_spotlights", api_py)
        self.assertIn("gold_pre_program_gate_summary", api_py)
        self.assertIn("gold_foundation_course_summary", api_py)
        self.assertIn("gold_milestone_traction_summary", api_py)
        self.assertIn("gold_project_learning_summary", api_py)
        self.assertIn("gold_top_viewed_activities", api_py)
        self.assertIn("gold_low_attention_activities", api_py)
        self.assertIn("gold_recent_submissions", api_py)
        self.assertIn("gold_submitted_projects", api_py)
        self.assertIn("gold_learning_activity_detail", api_py)
        self.assertIn("gold_ueh_lms_entrepreneurship_enrollment_detail", api_py)
    ...
    def test_live_moodle_log_ingestion_contract_is_available(self):
        ...
        self.assertTrue(Path("scripts/export_moodle_logs_once.py").exists())
        self.assertIn("MOODLE_UI_BASE_URL", Path(".env.example").read_text(encoding="utf-8"))
        self.assertIn("export_moodle_logs_once.py", Path("docs/plan/15-moodle-log-ingestion-architecture.md").read_text(encoding="utf-8"))
        export_script = Path("scripts/export_moodle_logs_once.py").read_text(encoding="utf-8")
        self.assertIn("api/v1/moodle-logs/import-csv", export_script)
        self.assertIn("ARCHIVE_DIR", export_script)
        self.assertIn("FAILED_DIR", export_script)
        self.assertTrue(Path("scripts/run-moodle-ui-log-pipeline-loop.ps1").exists())
        self.assertIn("run-moodle-ui-log-pipeline-loop.ps1", Path("docs/plan/15-moodle-log-ingestion-architecture.md").read_text(encoding="utf-8"))
        self.assertTrue(Path("scripts/run-local-analytics-refresh.ps1").exists())
        refresh_script = Path("scripts/run-local-analytics-refresh.ps1").read_text(encoding="utf-8")
        self.assertIn("dbt", refresh_script)
        self.assertIn("learning-dashboard-overview", refresh_script)
        self.assertIn("data_schema", refresh_script)
        self.assertIn("VerboseDbt", refresh_script)
        self.assertIn("--quiet", refresh_script)
        self.assertIn("run-local-analytics-refresh.ps1", Path("scripts/run-moodle-ui-log-pipeline-loop.ps1").read_text(encoding="utf-8"))
        self.assertTrue(Path("scripts/run-live-analytics-refresh.ps1").exists())
        live_refresh_script = Path("scripts/run-live-analytics-refresh.ps1").read_text(encoding="utf-8")
        self.assertIn("TARGET_DATABASE_URL", live_refresh_script)
        self.assertIn("RENDER_APP_BASE_URL", live_refresh_script)
        self.assertIn("DBT_POSTGRES_SSLMODE", live_refresh_script)
        self.assertIn("learning-dashboard-overview", live_refresh_script)
        self.assertIn("data_schema", live_refresh_script)
        self.assertIn("--quiet", live_refresh_script)
        self.assertIn("run-live-analytics-refresh.ps1", Path("docs/plan/14-demo-deploy-render-neon.md").read_text(encoding="utf-8"))
        self.assertIn("TARGET_DATABASE_URL", Path(".env.example").read_text(encoding="utf-8"))
        self.assertIn("RENDER_APP_BASE_URL", Path(".env.example").read_text(encoding="utf-8"))
        self.assertIn("sslmode", Path("analytics/dbt_thinkspace/profiles.example.yml").read_text(encoding="utf-8"))
        seed_neon_script = Path("scripts/seed-neon-demo.ps1").read_text(encoding="utf-8")
        self.assertIn("run-live-analytics-refresh.ps1", seed_neon_script)
        self.assertIn("SkipLiveAnalyticsRefresh", seed_neon_script)
        self.assertIn("analytics.gold_registered_user_learning_summary", seed_neon_script)
        self.assertNotIn("import src.app.main", seed_neon_script)
        self.assertNotIn("Tao lai cac view phan tich", seed_neon_script)
        self.assertIn("data/", Path(".gitignore").read_text(encoding="utf-8"))
        self.assertTrue(Path("analytics/dbt_thinkspace/models/silver/silver_moodle_learning_events.sql").exists())
        self.assertIn("progress_signal_type", Path("analytics/dbt_thinkspace/models/silver/silver_moodle_learning_events.sql").read_text(encoding="utf-8"))
        self.assertIn("silver:", Path("analytics/dbt_thinkspace/dbt_project.yml").read_text(encoding="utf-8"))
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_registered_user_learning_summary.sql").exists())
        self.assertIn("current_learning_status", Path("analytics/dbt_thinkspace/models/marts/learning/gold_registered_user_learning_summary.sql").read_text(encoding="utf-8"))
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_project_learning_summary.sql").exists())
        self.assertIn("current_project_learning_status", Path("analytics/dbt_thinkspace/models/marts/learning/gold_project_learning_summary.sql").read_text(encoding="utf-8"))
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_milestone_traction_summary.sql").exists())
        self.assertIn("submission_done_project_count", Path("analytics/dbt_thinkspace/models/marts/learning/gold_milestone_traction_summary.sql").read_text(encoding="utf-8"))
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/dim_moodle_course_activities.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_team_learning_summary.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_individual_learning_summary.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_daily_learning_interactions.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_key_activity_spotlights.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_pre_program_gate_summary.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_foundation_course_summary.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_ueh_lms_entrepreneurship_enrollment_summary.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_top_viewed_activities.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_low_attention_activities.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_recent_submissions.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_submitted_projects.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_learning_activity_detail.sql").exists())
        self.assertTrue(Path("analytics/dbt_thinkspace/models/marts/learning/gold_ueh_lms_entrepreneurship_enrollment_detail.sql").exists())
        self.assertIn("activity_detail_key", Path("analytics/dbt_thinkspace/models/marts/learning/gold_learning_activity_detail.sql").read_text(encoding="utf-8"))
        self.assertIn("raw_ueh_lms_course_enrollments", Path("analytics/dbt_thinkspace/models/sources.yml").read_text(encoding="utf-8"))
        self.assertIn("gold_key_activity_spotlights", Path("analytics/dbt_thinkspace/models/marts/learning/schema.yml").read_text(encoding="utf-8"))
        self.assertIn("marts:", Path("analytics/dbt_thinkspace/dbt_project.yml").read_text(encoding="utf-8"))
